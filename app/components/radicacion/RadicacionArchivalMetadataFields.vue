...
watch(
  ...
  async (id) => {
    ...
    loading.value = true
    try {
      resolvedSchema.value = await metaApi.resolveSchema(id)
    } catch {
      resolvedSchema.value = null
    } finally {
      loading.value = false
...
const activeFields = computed(() =>
  (resolvedSchema.value?.fields ?? []).filter((f: ArchivalMetadataFieldRow) => f.is_active !== false)
...
function fieldId(f: ArchivalMetadataFieldRow, idx: number) {
  return `meta_${f.code}_${idx}`
}
</script>

<template>
  <div v-if="loading" class="text-xs text-muted-foreground py-1">
    Cargando metadatos…
  </div>
  <div v-else-if="activeFields.length" class="mt-3 space-y-3 rounded-lg border border-dashed border-primary/30 bg-muted/15 p-3">
    <p class="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
      Metadatos archivísticos
      <span v-if="resolvedSchema" class="normal-case font-normal">({{ resolvedSchema.name }})</span>
    </p>
    <div
