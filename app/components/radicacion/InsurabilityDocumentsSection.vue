...
const allowsRemoveUploaded = computed(
  () =>
    ...
    && (hasPermission('radicacion_documentos_eliminar') || hasPermission('radicacion_insurability_documentos_eliminar')),
)
...
function patchInsurabilityDocumentsMap(next: Record<string, number | null>): void {
  ...
  patchApplicant({
    financial_info: {
      ...financial.value,
      insurabilityDocuments: { ...prev, ...next },
    },
  })
}
...
async function removeUploaded(key: string): Promise<void> {
  ...
  try {
    ...
    patchInsurabilityDocumentsMap({ [key]: null })
...
function pendingFileFor(key: string): File | undefined {
  const f = props.applicant.insurabilityDocumentFiles?.[key]
...
function patchInsurabilityFiles(next: Record<string, File | undefined>): void {
  const prev = props.applicant.insurabilityDocumentFiles ?? {}
  patchApplicant({ insurabilityDocumentFiles: { ...prev, ...next } })
}

async function loadChecklistConfig(): Promise<void> {
  loadingConfig.value = true
  try {
    const res = await $api<unknown>('/catalogs/template-flat-data/documentation-insurability-documents')
    checklistRows.value = extractInsurabilityItemsFromCatalogResponse(res)
  } catch (e) {
    console.error(e)
    toast.error(messageFromFetchError(e, 'No se pudo cargar el checklist de asegurabilidad.'))
    checklistRows.value = []
  } finally {
...
function safeInputId(key: string, index: number): string {
  ...
  return `ins_doc_${index}_${slug}`
}

function pickInsurabilityFile(key: string, file: File | undefined): void {
  if (uploadBlockedForKey(key)) {
    return
  }
  if (!file) {
  ...
  patchInsurabilityFiles({ [key]: file })
}
...
function clearPending(key: string): void {
  patchInsurabilityFiles({ [key]: undefined })
}
...
<template>
  <div id="radicacion-insurability-documents" class="space-y-4">
    <div>
      <h4 class="text-sm font-semibold">
        Documentos de asegurabilidad (parametrizados)
      </h4>
      <p class="mt-1 text-xs text-muted-foreground leading-relaxed">
        Lista definida en Parametrización → Radicación → Asegurabilidad. Mismo criterio de archivos que el módulo auxiliar (PDF, ZIP, imagen · máximo 10 MB). No requiere marcar «Revisado» en esta sección. Si ya hay archivo cargado, use «Reemplazar archivo» para cambiarlo; «Eliminar» requiere permiso de eliminación (adjuntos o solo asegurabilidad).
      </p>
    </div>
    ...
    >
      No hay documentos parametrizados para asegurabilidad. Revise Parametrización → Radicación → Asegurabilidad, o ejecute en la API
      <code class="rounded bg-muted px-1 py-0.5 text-xs">php artisan db:seed --class=DocumentationInsurabilityDocumentsTemplateSeeder</code>
            ...
            <template v-else-if="docMetaForKey(row.key)">
              ...
              >
                ...
                <button
                  ...
                  @click="void openInsurabilityDocumentInPreview(row.key)"
                >
