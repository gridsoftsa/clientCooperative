...
import {
  ...
  type ApproverEntityChecklistItem,
...
const checklistRows = ref<ApproverEntityChecklistItem[]>([])
...
const docIdsByKey = computed((): Record<string, number | null> =>
  parseFinancialChecklistDocumentIdMap(financial.value.approverEntityDocuments),
)
...
const allowsRemoveUploaded = computed(
  () =>
    ...
    && (hasPermission('radicacion_documentos_eliminar') || hasPermission('radicacion_approver_entity_documentos_eliminar')),
)

function patchApproverEntityDocumentsMap(next: Record<string, number | null>): void {
  const raw = financial.value.approverEntityDocuments
  ...
  patchApplicant({
    financial_info: {
      ...financial.value,
      approverEntityDocuments: { ...prev, ...next },
    },
  })
}
...
async function removeUploaded(key: string): Promise<void> {
  ...
  try {
    ...
    patchApproverEntityDocumentsMap({ [key]: null })
...
function pendingFileFor(key: string): File | undefined {
  const f = props.applicant.approverEntityDocumentFiles?.[key]
...
function patchApproverEntityFiles(next: Record<string, File | undefined>): void {
  const prev = props.applicant.approverEntityDocumentFiles ?? {}
  patchApplicant({ approverEntityDocumentFiles: { ...prev, ...next } })
}

async function loadChecklistConfig(): Promise<void> {
  loadingConfig.value = true
  try {
    const res = await $api<unknown>('/catalogs/template-flat-data/credit-director-approver-documents')
    checklistRows.value = extractApproverEntityItemsFromCatalogResponse(res)
  } catch (e) {
    console.error(e)
    toast.error(messageFromFetchError(e, 'No se pudo cargar el checklist del ente aprobador.'))
    checklistRows.value = []
  } finally {
...
function safeInputId(key: string, index: number): string {
  ...
  return `app_ent_doc_${index}_${slug}`
}

function pickApproverEntityFile(key: string, file: File | undefined): void {
  if (uploadBlockedForKey(key)) {
    return
  }
  if (!file) {
  ...
  patchApproverEntityFiles({ [key]: file })
}
...
function onFileInput(key: string, event: Event): void {
  ...
  input.value = ''
  pickApproverEntityFile(key, file)
}
...
function onAppDragOverWrapper(key: string, e: DragEvent): void {
  if (uploadBlockedForKey(key)) {
    return
  }
  onInsDragOver(e)
...
function onAppDropWrapper(key: string, e: DragEvent): void {
  if (uploadBlockedForKey(key)) {
    return
  }
  e.preventDefault()
...
function onAppKeyWrapper(key: string, idx: number, e: KeyboardEvent): void {
  if (uploadBlockedForKey(key)) {
    return
  }
  onAppUploadZoneActivate(key, idx, e)
}

function onAppUploadZoneActivate(key: string, idx: number, event?: MouseEvent | KeyboardEvent): void {
  if (uploadBlockedForKey(key)) {
    return
  }
...
function clearPending(key: string): void {
  patchApproverEntityFiles({ [key]: undefined })
}

async function openApproverEntityDocumentInPreview(key: string): Promise<void> {
...
<template>
  <div id="radicacion-approver-entity-documents" class="space-y-4">
    <div>
      <h4 class="text-sm font-semibold">
        Documentos del ente aprobador (parametrizados)
      </h4>
      <p class="mt-1 text-xs text-muted-foreground leading-relaxed">
        Lista definida en Parametrización → Radicación → Aprobadores. Mismo criterio de archivos que el módulo auxiliar (PDF, ZIP, imagen · máximo 10 MB). «Eliminar» requiere permiso de eliminación (adjuntos o documentos del ente aprobador).
      </p>
    </div>
    ...
    >
      No hay documentos parametrizados para el ente aprobador. Revise Parametrización → Radicación → Aprobadores, o ejecute en la API
      <code class="rounded bg-muted px-1 py-0.5 text-xs">php artisan db:seed --class=CreditDirectorApproverDocumentsTemplateSeeder</code>
          ...
          <div class="p-4 sm:p-5">
            ...
            <template v-if="pendingFileFor(row.key)">
              <div
                ...
                @click="onAppUploadZoneActivate(row.key, idx, $event)"
                @keydown.enter.prevent="onAppKeyWrapper(row.key, idx, $event)"
                @keydown.space.prevent="onAppKeyWrapper(row.key, idx, $event)"
                @dragover="onAppDragOverWrapper(row.key, $event)"
                @drop="onAppDropWrapper(row.key, $event)"
              >
                ...
                <div class="flex flex-wrap items-center justify-center gap-x-4 gap-y-2">
                  <button
                    ...
                    @click.stop.prevent="void openApproverEntityDocumentInPreview(row.key)"
                  >
                    Ver
                  </button>
                  <button
            ...
            <template v-else-if="docMetaForKey(row.key)">
              ...
              >
                ...
                <button
                  ...
                  @click="void openApproverEntityDocumentInPreview(row.key)"
                >
