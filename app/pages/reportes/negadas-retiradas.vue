...
interface NegadasResponse {
  data: {
    ...
    summary?: {
      ...
      rejected_amount_sum: string
      cancelled_amount_sum: string
    }
    rows?: NegadasDetailRow[]
  }
}
...
const reportData = ref<NegadasResponse['data'] | null>(null)
...
async function exportReport(format: 'xlsx' | 'pdf'): Promise<void> {
  ...
  try {
    ...
    const base = `reporte-negadas-retiradas-${stamp}`
    if (format === 'xlsx') {
      await downloadReportFile(
        '/reports/negadas-retiradas/export',
        q,
        `${base}.xlsx`,
...
async function fetchReport(): Promise<void> {
  ...
  loading.value = true
  try {
    const res = await $api<NegadasResponse>('/reports/negadas-retiradas', {
  ...
  <div class="flex w-full flex-col gap-5">
    ...
    <div>
      ...
      <p class="text-sm text-muted-foreground">
        Solicitudes en estado Rechazada (director de crédito) o Cancelada (retirada). Elija vista consolidada o detalle con radicado, decisión, fecha y usuario.
      </p>
    </div>
    ...
    <Card class="shadow-sm">
      <CardHeader class="flex flex-col gap-4 space-y-0 pb-3 sm:flex-row sm:items-start sm:justify-between">
        <div class="space-y-1">
          ...
          <CardDescription>
            Período por <code class="rounded bg-muted px-1 py-0.5 text-[11px]">created_at</code> de la radicación; sucursal opcional.
          </CardDescription>
        </div>
          ...
          <div class="flex flex-col gap-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:max-w-2xl">
              <div class="min-w-0 space-y-1.5">
                <Label for="report-negadas-sucursal">Sucursal de la radicación</Label>
                <Select
                  :model-value="sucursalId == null ? 'all' : String(sucursalId)"
                  :disabled="loadingSucursales"
                  @update:model-value="sucursalId = $event === 'all' ? null : Number($event)"
                >
                  <SelectTrigger id="report-negadas-sucursal" class="w-full">
              ...
              <div class="min-w-0 space-y-1.5">
                <Label for="report-negadas-mode">Tipo de informe</Label>
                <Select
                  :model-value="reportMode"
                  @update:model-value="reportMode = $event as ReportConsolidationMode"
                >
                  <SelectTrigger id="report-negadas-mode" class="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem
        ...
        <template v-else-if="reportData">
              ...
              <Card>
                <CardHeader class="space-y-1 px-4 py-4">
                  ...
                  <CardTitle class="text-2xl tabular-nums">
                    {{ reportData.summary.rejected_count }}
                  </CardTitle>
                  <p class="text-sm text-muted-foreground">
                    Suma: {{ formatPesosConSimboloDesdeTexto(reportData.summary.rejected_amount_sum) }}
                  </p>
                </CardHeader>
              </Card>
              <Card>
                <CardHeader class="space-y-1 px-4 py-4">
                  <CardDescription>Canceladas (retiradas)</CardDescription>
                  <CardTitle class="text-2xl tabular-nums">
                    {{ reportData.summary.cancelled_count }}
                  </CardTitle>
                  <p class="text-sm text-muted-foreground">
                    Suma: {{ formatPesosConSimboloDesdeTexto(reportData.summary.cancelled_amount_sum) }}
                  </p>
                </CardHeader>
              </Card>
            </div>
          ...
          <template v-else-if="reportData.filters?.mode === REPORT_MODE_DETAIL">
            <div v-if="!reportData.rows?.length" class="rounded-lg border border-dashed py-8 text-center text-sm text-muted-foreground">
              No hay radicaciones rechazadas o canceladas en el período y filtros.
            </div>
