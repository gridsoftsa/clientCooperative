...
interface PrivilegiadosDetailRow {
  code: string
...
interface PrivilegiadosResponse {
  data: {
    report: string
    filters: {
      created_from: string
...
const reportData = ref<PrivilegiadosResponse['data'] | null>(null)
...
async function exportReport(format: 'xlsx' | 'pdf'): Promise<void> {
  ...
  try {
    ...
    const base = `reporte-privilegiados-${stamp}`
    if (format === 'xlsx') {
      await downloadReportFile(
        '/reports/privilegiados/export',
        q,
        `${base}.xlsx`,
...
async function fetchReport(): Promise<void> {
  ...
  loading.value = true
  try {
    const res = await $api<PrivilegiadosResponse>('/reports/privilegiados', {
  ...
  <div class="flex w-full flex-col gap-5">
    ...
    <div>
      ...
      <p class="text-sm text-muted-foreground">
        Radicaciones marcadas como privilegiadas. Consolidado con totales o detalle con justificación (todos los fragmentos), origen del registro, fecha y usuario.
      </p>
    </div>
          ...
          <div class="flex flex-col gap-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:max-w-2xl">
              <div class="min-w-0 space-y-1.5">
                <Label for="report-privilegiados-sucursal">Sucursal de la radicación</Label>
                <Select
                  :model-value="sucursalId == null ? 'all' : String(sucursalId)"
                  :disabled="loadingSucursales"
                  @update:model-value="sucursalId = $event === 'all' ? null : Number($event)"
                >
                  <SelectTrigger id="report-privilegiados-sucursal" class="w-full">
              ...
              <div class="min-w-0 space-y-1.5">
                <Label for="report-privilegiados-mode">Tipo de informe</Label>
                <Select
                  :model-value="reportMode"
                  @update:model-value="reportMode = $event as ReportConsolidationMode"
                >
                  <SelectTrigger id="report-privilegiados-mode" class="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem
        ...
        <template v-else-if="reportData">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
              <Card class="border-primary/20 bg-primary/5">
                <CardHeader class="space-y-1 px-4 py-4">
                  <CardDescription>Total privilegiadas</CardDescription>
              ...
              <Card>
                <CardHeader class="space-y-1 px-4 py-4">
                  ...
                  <CardTitle class="text-2xl tabular-nums">
                    {{ formatPesosConSimboloDesdeTexto(reportData.summary.amount_sum) }}
                  </CardTitle>
                </CardHeader>
              </Card>
            </div>
          ...
          <template v-else-if="reportData.filters?.mode === REPORT_MODE_DETAIL">
            <div v-if="!reportData.rows?.length" class="rounded-lg border border-dashed py-8 text-center text-sm text-muted-foreground">
              No hay radicaciones privilegiadas en el período y filtros.
            </div>
