...
const reportData = ref<AnalisisResponse['data'] | null>(null)
...
async function fetchReport(): Promise<void> {
  ...
  loading.value = true
  try {
    const res = await $api<AnalisisResponse>('/reports/analisis-in-analysis', {
      query: buildReportQuery(),
    })
    reportData.value = res.data
  } catch (error: any) {
    console.error('Error cargando reporte de análisis:', error)
...
async function exportReport(format: 'xlsx' | 'pdf'): Promise<void> {
  ...
  try {
    ...
    const base = `reporte-analisis-in-analysis-${stamp}`
    if (format === 'xlsx') {
      await downloadReportFile(
        '/reports/analisis-in-analysis/export',
        q,
        `${base}.xlsx`,
        ...
        <div class="rounded-xl border bg-muted/30 px-4 py-4">
          <div class="flex w-full flex-col gap-4 lg:flex-row lg:items-end lg:gap-6">
            ...
            <div class="w-full space-y-1.5 lg:w-72 lg:shrink-0">
              ...
              >
                <SelectTrigger id="report-analisis-sucursal" class="w-full">
